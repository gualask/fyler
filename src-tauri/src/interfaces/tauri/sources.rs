...
use crate::infrastructure::source_registry::{SourceRegistry, IMAGE_EXTENSIONS};
use crate::modules::sources::{
    discard_pending_sources as discard_pending_sources_use_case, files_from_paths_with_progress,
    image_preview, image_preview_for_path, release_sources as release_sources_use_case,
    unlock_pdf_source as unlock_pdf_source_use_case, FilesFromPathsResult, ImportProgress,
    OpenFilesResult, SkippedFile, SkippedFileReason, SourceFile, SourceLifecycle,
};
...
pub(super) fn emit_import_progress(app: &tauri::AppHandle, progress: ImportProgress) {
...
pub async fn unlock_pdf_source(
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        unlock_pdf_source_use_case(&task_registry, path, password)
    })
    .await;
    match result {
...
pub fn discard_pending_sources(paths: Vec<String>, registry: State<'_, SourceRegistry>) {
    discard_pending_sources_use_case(registry.inner(), &paths);
}
...
pub fn release_sources(file_ids: Vec<String>, registry: State<'_, SourceRegistry>) {
    release_sources_use_case(registry.inner(), &file_ids);
}

#[tauri::command]
/// Returns a retained import preview or generates a bounded preview for an authorized path.
pub async fn get_image_preview(
    ...
    let bytes = match (original_path, max_side) {
        (Some(path), Some(max_side)) => {
            if !app.asset_protocol_scope().is_allowed(&path) {
                return Err(source_not_found_error().into());
            }
            let registry = registry.inner().clone();
            tauri::async_runtime::spawn_blocking(move || {
                image_preview_for_path(&registry, &path, max_side.clamp(32, 1600))
            })
            .await
            .map_err(anyhow::Error::from)??
        }
        _ => image_preview(registry.inner(), &file_id),
    };
