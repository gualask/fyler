...
use super::super::{
    discard_pending_sources, files_from_paths, unlock_pdf_source, SourceLifecycle, SourceLookup,
};
use super::temp_path;
use crate::capabilities::pdf::metadata::{
    count_pages, count_pages_with_password, is_password_required_error,
};
...
fn protected_pdf_stays_deduplicated_until_unlocked_or_skipped() -> anyhow::Result<()> {
    ...
    let unlocked = unlock_pdf_source(
        &registry,
        encrypted_path.clone(),
        TEST_PDF_PASSWORD.to_string(),
    )?;
    assert_eq!(unlocked.original_path, encrypted_path);
    assert!(registry.get(&unlocked.id).is_some());
