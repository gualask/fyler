...
use super::OptimizationOptions;
use crate::capabilities::raster_compression;
use crate::capabilities::raster_compression::AUTOMATIC_LOSSY_QUALITY;
...
pub fn should_keep_original(original_size: usize, rewritten_size: usize) -> bool {
    raster_compression::should_keep_original(original_size, rewritten_size)
}
...
fn choose_output_encoding(
    ...
    if opts.target_dpi.is_some() {
        if dimensions != (candidate.width, candidate.height) || should_auto_jpeg_raw(candidate) {
            return Some(OutputEncoding::Jpeg(AUTOMATIC_LOSSY_QUALITY));
        }
        return None;
    }
...
#[cfg(test)]
mod tests {
    use super::{choose_output_encoding, ImageCandidate, OptimizationOptions, OutputEncoding};
    use crate::capabilities::pdf::optimization::candidate::{SourceEncoding, SupportedColorSpace};

    fn candidate() -> ImageCandidate {
        ImageCandidate {
            object_id: (1, 0),
            width: 2_000,
            height: 1_000,
            color_space: SupportedColorSpace::Rgb,
            source_encoding: SourceEncoding::Jpeg,
            original_size: 200_000,
        }
    }

    #[test]
    fn automatic_reencoding_uses_shared_quality_92() {
        assert_eq!(
            ...
            Some(OutputEncoding::Jpeg(92))
        );
    }

    #[test]
    fn manual_quality_remains_an_explicit_override() {
        assert_eq!(
            choose_output_encoding(
                &candidate(),
                &OptimizationOptions {
