...
test('selects the first true remaining file when removing a batch with the current selection', () => {
    const state = workspaceUiReducer(
        {
            selectedId: 'selected-file',
            selectedFileScrollKey: 5,
        ...
        {
            type: 'files-removed',
            fileIds: ['quick-file-a', 'selected-file'],
            remainingIds: ['existing-file', 'later-file'],
        },
    );

    assert.deepEqual(state, {
        selectedId: 'existing-file',
        selectedFileScrollKey: 6,
        focusedSource: null,
        uiSignalKey: 6,
    });
});

test('clears focused source when removing a batch without changing an unaffected selection', () => {
    const state = workspaceUiReducer(
        {
            selectedId: 'existing-file',
            selectedFileScrollKey: 7,
            focusedSource: {
                fileId: 'quick-file',
                target: { kind: 'image' },
                flashKey: 7,
