...
function resolveAccentColor(accent: unknown): AccentColor {
    if (accent === 'amber' || accent === 'fuchsia' || accent === 'lime') {
        return DEFAULT_ACCENT_COLOR;
    }
    return isAccentColor(accent) ? accent : DEFAULT_ACCENT_COLOR;
}
...
export function resolvePreferencesState(
    ...
    return {
        ...
        accent: resolveAccentColor(settings?.accent),
