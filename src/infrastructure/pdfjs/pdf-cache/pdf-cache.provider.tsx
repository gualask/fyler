...
import { useDocumentPreviewPort } from '@/capabilities/document-preview/preview.port';
...
function usePdfDocumentAccess(refs: PdfCacheRefs) {
    ...
    const getPdfDocument = useCallback(
        (file: SourceFile) =>
            getOrCreatePdfDocument(file, pdfDocumentCaches(refs), documentPreview),
        [documentPreview, refs],
    );
...
function usePdfCacheValue(
    requestRenders: PdfCacheContextType['requestRenders'],
    getPageAspectRatio: PdfCacheContextType['getPageAspectRatio'],
    getPdfDocument: PdfCacheContextType['getPdfDocument'],
    setPdfPassword: PdfCacheContextType['setPdfPassword'],
    releaseFile: PdfCacheContextType['releaseFile'],
): PdfCacheContextType {
    return useMemo(
        () => ({
            requestRenders,
        ...
        [getPageAspectRatio, getPdfDocument, releaseFile, requestRenders, setPdfPassword],
    );
}
...
export function PdfCacheProvider({ children }: { children: ReactNode }) {
    ...
    return <PdfCacheContext.Provider value={value}>{children}</PdfCacheContext.Provider>;
}
