...
export function useProtectedPdfSkipActions({
    ...
    const skipCurrent = useCallback(() => {
        ...
        void documentSources.discardPendingSources([activeImport.current.originalPath]);
        skipCurrentPasswordFile(activeImport.pending);
        showNextOrFinish(activeImport.pending);
    }, [documentSources, isChecking, pendingRef, record, showNextOrFinish, unlockInFlightRef]);
    ...
    const skipAll = useCallback(() => {
        ...
        void documentSources.discardPendingSources(pending.queue.map((file) => file.originalPath));
        pending.queue = [];
        finishPending();
    }, [documentSources, finishPending, isChecking, pendingRef, record, unlockInFlightRef]);
