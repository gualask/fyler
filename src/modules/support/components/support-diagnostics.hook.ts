...
import { type SupportPort, useSupportPort } from '../support.port';
...
function useAppMetadata(record: RecordDiagnostic, supportPort: SupportPort): AppMetadata {
...
async function saveDiagnosticsReportFile(
    ...
    try {
        const path = await supportPort.saveTextFile(
            defaultFilename,
            filterLabel,
            diagnosticsReport,
        );
...
async function openGitHubIssueTarget(
    { title, body }: GitHubIssueDraft,
    record: RecordDiagnostic,
    supportPort: SupportPort,
) {
    const target = buildGitHubIssueOpenTarget({ title, body });
    try {
        await supportPort.openExternalUrl(target.url);
        record({
            category: 'support',
            severity: 'info',
            message:
...
export function useSupportDiagnostics({
    ...
    const supportPort = useSupportPort();
    const appMetadata = useAppMetadata(record, supportPort);
    ...
    const saveDiagnosticsFile = useCallback(
        async (options: SaveDiagnosticsOptions) => {
            return saveDiagnosticsReportFile(
                options,
        ...
        [buildDiagnosticsReport, record, supportPort],
    );

    const openGitHubIssue = useCallback(
        async (draft: GitHubIssueDraft) => {
            return openGitHubIssueTarget(draft, record, supportPort);
        },
        [record, supportPort],
    );
